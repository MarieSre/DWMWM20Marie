 ...
 *
 * @param array $plateau
 * @param string $jeu Si l'utilisateur veut jouer au morpion : M, sinon P pour le puissance 4
 * @return void
 */
...
function conversionPosition($coordonnee, $jeu)
{
    if ($jeu == "M") // Morpion
    {
        ...
        $tabCord[1] = $numCol;
        return $tabCord;
    } else // Puissance 4 (On ne reçoit que la colonne)
    {
...
function selectionPosition($plateau, $symbole, $jeu)
{
    ...
    } else // Puissance 4
    {
        do {
            ...
            } while ($col >= count($plateau[0]));
        } while ($plateau[$lig[0]][$col] != '.');
        return $positions;
    }
}
...
{
    for ($i = count($plateau); $i < count($plateau[0]); $i--) {
        if ($numCol == 0) {
...
{
    $jeu = strtoupper(readline("A quoi voulez vous jouer ? (M/P) "));
    if ($jeu == "M") // Morpion
    {
